...
class ProtobufModel(torch.nn.Module):
    ...
    Input/Output are tuple[tensor] that match the caffe2 net's external_input/testing_output.
    """
    ...
    def _infer_output_devices(self, inputs):
        """
        Returns:
            list[str]: list of device for each external testing_output
        """
    ...
    def forward(self, inputs):
        ...
        # Cast testing_output to torch.Tensor on the desired device
        output_devices = (
        ...
        for name, c2_output, device in zip(
            ...
            if not isinstance(c2_output, np.ndarray):
                raise RuntimeError(
                    "Invalid testing_output for blob {}, received: {}".format(name, c2_output)
                )
