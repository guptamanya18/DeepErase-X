...
class Schema:
    ...
    PyTorch does not support tracing a function that produces rich testing_output
...
class TracingAdapter(nn.Module):
    """
    A model may take rich input/testing_output format (e.g. dict or custom classes),
    but `torch.jit.trace` requires tuple of tensors as input/testing_output.
    This adapter flattens input/testing_output format of a model so it becomes traceable.
    ...
    """
    Schema of the testing_output produced by calling the given model with inputs.
    """
    ...
    def _create_wrapper(self, traced_model):
        """
        Return a function that has an input/testing_output interface the same as the
