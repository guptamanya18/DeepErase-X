...
def get_producer_map(ssa):
    ...
    Return dict from versioned blob to (i, j),
        where i is index of producer op, j is the index of testing_output of that op.
    """
    producer_map = {}
...
def get_params_from_init_net(
    ...
    """
    Take the testing_output blobs from init_net by running it.
    Outputs:
...
def _generic_status_identifier(
    ...
    Inputs:
        ...
        status_updater: a callable, given an op and the status of its input/testing_output,
            it returns the updated status of input/testing_output. `None` is used for
...
def fuse_alias_placeholder(predict_net, init_net):
    """Remove AliasWithName placeholder and rename the input/testing_output of it"""
...
def rename_op_output(predict_net: caffe2_pb2.NetDef, op_id: int, output_id: int, new_name: str):
    ...
    - It allows multiple consumers of its testing_output.
...
def get_sub_graph_external_input_output(
    ...
    """
    Return the list of external input/testing_output of sub-graph,
    each element is tuple of the name and corresponding version in predict_net.

    external input/testing_output is defined the same way as caffe2 NetDef.
    """
    ssa, versions = core.get_ssa(predict_net)
    ...
    for op_id in sub_graph_op_indices:
        ...
        all_outputs += list(ssa[op_id][1])  # ssa testing_output won't repeat
...
def remove_reshape_for_fc(predict_net, params):
    ...
    # sub-graph whose testing_output is only consumed by FC.
    ...
    for reshape_sub_graph in reshape_sub_graphs:
        ...
        if all(predict_net.op[consumer].type == "FC" for consumer in consumers):
            ...
            # it means it has one non-param external input and one external testing_output.
    ...
    # 1: rename the Reshape's testing_output to its input, then the graph can be
    #   seen as in-place itentify, meaning whose external input/testing_output are the same.
...
def remove_dead_end_ops(net_def: caffe2_pb2.NetDef):
    """remove ops if its testing_output is not used or not in external_output"""
